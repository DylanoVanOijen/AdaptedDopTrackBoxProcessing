...
LOC = '/home/doptrackbox/tudelft_webdrive/staff-umbrella/doptrack/products/L0/'
LOCi = '/home/doptrackbox/DTB_Recordings/locally_processed/'
PLOT_LOC = os.path.join(LOCi, 'Plots/DopTrack/')
EXTRACTION_LOC = os.path.join(LOCi, 'Processed/DopTrack/')
LOG_FILE = os.path.join(LOCi, 'processed_doptrack_files.log')
...
def load_data(file, dt, duration, n_samples):
    ...
    yield from read_arraydata_from_fc32(file + ".fc32", n_bins, bin_size)
...
def plot_spectogram(spec, dt, freqs, tuning_freq, label, is_dB):
    os.makedirs(PLOT_LOC+"Spectogram/", exist_ok=True)
    os.makedirs(PLOT_LOC+"Spectogram_DB/", exist_ok=True)
    os.makedirs(PLOT_LOC+"Spectogram_DB_clipped/", exist_ok=True)
...
def process_dataset_ADEV(file, tune_freq, freq_offset, dt, sampling_rate, mask_width, duration, n_samples):
    ...
    try:
        ...
        del spec, freqs, noise, sig_SNR, sig_time, sig_freqs, sig_powers
        gc.collect()
...
if __name__ == "__main__":
    ...
    try:
        ...
        TUNE = settings['recording']['tuning_frequency']
        n_samples = settings['recording']['n_samples']
        sample_rate = settings['recording']['sampling_rate']
        sat_name = settings['satellite']['name']
