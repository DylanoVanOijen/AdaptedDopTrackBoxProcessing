...
def plot_raw_freq(DT_time, DT_freq, DTB_time, DTB_freq, DTB_snr, tune, file, interpolation_function):
    ...
    if RECOMPUTE_TIMESTAMPS:
        os.makedirs(PLOT_LOC+"DTBvsDT_freq_recomputed_timestamps", exist_ok=True)
        fig_1.savefig(os.path.join(PLOT_LOC, f"DTBvsDT_freq_recomputed_timestamps/freq_recomputed_timestamps_{file}.png"))
    else:       
        os.makedirs(PLOT_LOC+"DTBvsDT_freq", exist_ok=True)
        fig_1.savefig(os.path.join(PLOT_LOC, f"DTBvsDT_freq/freq_{file}.png"))
    plt.close(fig_1)
    gc.collect()
    ...
    fig_2.tight_layout()
    if RECOMPUTE_TIMESTAMPS:
        os.makedirs(PLOT_LOC+"DTBvsDT_freq_diff_recomputed_timestamps", exist_ok=True)
        fig_2.savefig(os.path.join(PLOT_LOC, f"DTBvsDT_freq_diff_recomputed_timestamps/freq_recomputed_timestamps_{file}.png"))
    else:       
        os.makedirs(PLOT_LOC+"DTBvsDT_freq_diff", exist_ok=True)
        fig_2.savefig(os.path.join(PLOT_LOC, f"DTBvsDT_freq_diff/freq_{file}.png"))
    plt.close(fig_2)
    ...
    if RECOMPUTE_TIMESTAMPS:
        os.makedirs(PLOT_LOC+"DTBvsDT_comp_recomputed_timestamps", exist_ok=True)
        fig_3.savefig(os.path.join(PLOT_LOC, f"DTBvsDT_comp_recomputed_timestamps/comp_recomputed_timestamps_{file}.png"))
    else:       
        os.makedirs(PLOT_LOC+"DTBvsDT_comp", exist_ok=True)
        fig_3.savefig(os.path.join(PLOT_LOC, f"DTBvsDT_comp/comp_{file}.png"))
...
def plot_fitted_freq(DT_time, DT_freq, DTB_time, DTB_freq, DTB_snr, tune, file, delta_t, delta_f, interpolation_function):
    ...
    fig_1.tight_layout()
    if RECOMPUTE_TIMESTAMPS:
        os.makedirs(PLOT_LOC+"DTBvsDT_fitted_freq_recomputed_timestamps", exist_ok=True)
        fig_1.savefig(os.path.join(PLOT_LOC, f"DTBvsDT_fitted_freq_recomputed_timestamps/shifted_freq_recomputed_timestamps_{file}.png"))        
    else:
        os.makedirs(PLOT_LOC+"DTBvsDT_fitted_freq", exist_ok=True)
        fig_1.savefig(os.path.join(PLOT_LOC, f"DTBvsDT_fitted_freq/shifted_freq_{file}.png"))
    plt.close(fig_1)
    gc.collect()
    ...
    fig_2.tight_layout()
    if RECOMPUTE_TIMESTAMPS:
        os.makedirs(PLOT_LOC+"DTBvsDT_fitted_freq_diff_recomputed_timestamps", exist_ok=True)
        fig_2.savefig(os.path.join(PLOT_LOC, f"DTBvsDT_fitted_freq_diff_recomputed_timestamps/shifted_freq_recomputed_timestamps_{file}.png"))        
    else:
        os.makedirs(PLOT_LOC+"DTBvsDT_fitted_freq_diff", exist_ok=True)
        fig_2.savefig(os.path.join(PLOT_LOC, f"DTBvsDT_fitted_freq_diff/shifted_freq_{file}.png"))
    plt.close(fig_2)
    ...
    if RECOMPUTE_TIMESTAMPS:
        os.makedirs(PLOT_LOC+"DTBvsDT_comp_fitted_recomputed_timestamps", exist_ok=True)
        fig_3.savefig(os.path.join(PLOT_LOC, f"DTBvsDT_comp_fitted_recomputed_timestamps/shifted_freq_recomputed_timestamps_{file}.png"))        
    else:
        os.makedirs(PLOT_LOC+"DTBvsDT_comp_fitted", exist_ok=True)
        fig_3.savefig(os.path.join(PLOT_LOC, f"DTBvsDT_comp_fitted/shifted_freq_{file}.png"))
    plt.close(fig_3)
    gc.collect()    
...
def save_fit_params_yaml(delta_t, delta_f, delta_t_uncertainty, delta_f_uncertainty, file, recomputed):
    ...
    if recomputed:
        outpath = EXTRACTION_LOC + "/FittedBiasRecomputed/"
    else:
        outpath = EXTRACTION_LOC + "/FittedBias/"
    ...
    try:    
        ...
        for RECOMPUTE_TIMESTAMPS in [True, False]:
            ...
            if DT_freq_data is not None:
                ...
                plot_raw_freq(DT_freq_data[:,0], DT_freq_data[:,1], DTB_data[:,0], DTB_data[:,1], DTB_data[:,4], TUNE, FILE, interpolation_function)

                params0 = [0.0]  # [delta_t, delta_f]
                #lower = [-6, -50]
                #upper = [6, 50]
                lower = [-10]
                upper = [10]
                result = least_squares(data_shift_residuals, params0, bounds=(lower, upper), jac='3-point', args=(DTB_data[:,0], DTB_data[:,1], interpolation_function, DT_freq_data[:,0], DT_tca_wrt_start))
                delta_t = result.x[0]
                delta_f = 0.0
                ...
                delta_t_uncert = param_uncertainties[0]
                delta_f_uncert = 0.0
                ...
                plot_fitted_freq(DT_freq_data[:,0], DT_freq_data[:,1], DTB_data[:,0], DTB_data[:,1], DTB_data[:,4], TUNE, FILE, delta_t, delta_f, interpolation_function)
